...
import './styles/InativosDashboardVIRTELECOM.css';

const InativosDashboardVIRTELECOM = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('https://apidoixc.nexusnerds.com.br/Data/clientes_inativosVIR.json');
  ...
  return (
    <div className="inativos-dashboard-container">
      <h2>Dashboard - Inativos VIRTELECOM</h2>
      <p>Total de Contratos Inativos: {total}</p>
      {loading ? (
...
export default InativosDashboardVIRTELECOM;
